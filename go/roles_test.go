...
// TestRoles_CreateForwardsRequiredMFA is REMOVED (ADR-101): the field it
// forwarded no longer exists on either side of the wire. Zero realms ever
// configured a per-role MFA floor.
...
func TestRoles_CreateForwardsAssignableTo(t *testing.T) {
	mux := http.NewServeMux()
	mintPlatformToken(mux)
	mux.HandleFunc("/platforms/"+testRealmID+"/roles", func(w http.ResponseWriter, r *http.Request) {
		raw, _ := io.ReadAll(r.Body)
		...
		if _, present := got["can_invite_roles"]; present {
			t.Error("can_invite_roles is still on the wire; ADR-101 retired it")
		}
		w.WriteHeader(201)
		_ = json.NewEncoder(w).Encode(map[string]any{
			"id": "role-bot", "name": "bot", "display_name": "Bot",
			"permissions": []string{}, "assignable_to": []string{"service"},
			"is_system": false, "created_at": 1, "updated_at": 1,
		})
	})
	srv := httptest.NewServer(mux)
