 ...
 * {@code {id?, display_name, signup_mode?, created_at?, owner}}.
 ...
 *        Under {@code allowlist} the domains that auto-provision are
 *        {@code tenant_domains} grants (ADR-094), claimed and proven through
 *        the domains API — they are not settable here.
...
public record TenantCreate(
    ...
    public static TenantCreate of(String displayName, TenantOwner owner) {
        return new TenantCreate(null, displayName, null, null, owner);
    }
    ...
    public static TenantCreate withId(String id, String displayName, TenantOwner owner) {
        return new TenantCreate(id, displayName, null, null, owner);
    }

    /** Returns a copy of this payload with {@code createdAt} set. */
    public TenantCreate withCreatedAt(String createdAt) {
        return new TenantCreate(id, displayName, signupMode, createdAt, owner);
    }
}
